...
class MultiAgentComparisonRunner:
    ...
    async def compare(self, queries: list[str]) -> list[ComparisonResult]:
        ...
        for query in queries:
            ...
            results.append(
                ComparisonResult(query=query, single_agent=single_resp, multi_agent=multi_resp)
            )
        return results
    ...
    def print_results(results: list[ComparisonResult]) -> None:
        ...
        for r in results:
            ...
            print(f"  {'-' * 20} {'-' * 30} {'-' * 30}")
            print(
                f"  {'Steps':<20} {r.single_agent.steps_taken:<30} {r.multi_agent.steps_taken:<30}"
            )
            print(
                f"  {'Tokens':<20} {r.single_agent.token_usage.total_tokens:<30} {r.multi_agent.token_usage.total_tokens:<30}"
            )
            print(
                f"  {'Latency (ms)':<20} {r.single_agent.latency_ms:<30.0f} {r.multi_agent.latency_ms:<30.0f}"
            )
            print(
                f"  {'Confidence':<20} {r.single_agent.confidence:<30.2f} {r.multi_agent.confidence:<30.2f}"
            )
            print()
        ...
        print(
            f"  Multi overhead: {m_tokens - s_tokens} tokens ({(m_tokens / max(s_tokens, 1) - 1) * 100:.0f}% more)"
        )
        print(f"{'=' * 80}\n")
