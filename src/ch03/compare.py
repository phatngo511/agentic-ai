...
class ComparisonRunner:
    ...
    def print_results(results: list[ComparisonResult]) -> None:
        ...
        for r in results:
            ...
            print(
                f"  {'Tokens':<20} {r.workflow.token_usage.total_tokens:<30} {r.agent.token_usage.total_tokens:<30}"
            )
            print(
                f"  {'Latency (ms)':<20} {r.workflow.latency_ms:<30.0f} {r.agent.latency_ms:<30.0f}"
            )
            print(
                f"  {'Confidence':<20} {r.workflow.confidence:<30.2f} {r.agent.confidence:<30.2f}"
            )
            print(
                f"  {'Escalated':<20} {str(r.workflow.escalated):<30} {str(r.agent.escalated):<30}"
            )
            print()
