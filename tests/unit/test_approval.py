...
async def test_auto_approve_high_confidence(default_policy, mock_provider):
    ...
    request = ApprovalRequest(action="search", description="Search docs", confidence=0.95, risk_level="low")
...
async def test_require_review_low_confidence(default_policy, mock_provider):
    gate = ApprovalGate(policy=default_policy, provider=mock_provider)
    request = ApprovalRequest(action="restart", description="Restart service", confidence=0.5, risk_level="low")
    response = await gate.check(request)
...
async def test_always_require_for_high_risk(default_policy, mock_provider):
    gate = ApprovalGate(policy=default_policy, provider=mock_provider)
    request = ApprovalRequest(action="delete", description="Delete data", confidence=0.99, risk_level="high")
    response = await gate.check(request)
...
async def test_rejection():
    ...
    request = ApprovalRequest(action="deploy", description="Deploy change", confidence=0.5, risk_level="medium")
    response = await gate.check(request)
